...
// Modelo Claude configurable por env — default a Sonnet (más económico)
...
// ── CLIENTES — Supabase primero, .md como fallback ────────────
async function loadClientFromDB(slug) {
  try {
    const { data, error } = await getDB().from('studio_clients').select('*').eq('slug', slug).single();
...
async function listClientsAll() {
  // Clientes en Supabase + .md files, deduplicados
  const fromFiles = listClients();
  try {
    const { data } = await getDB().from('studio_clients').select('slug, name, rubro');
    const dbSlugs = (data || []).map(r => r.slug);
    const combined = [...new Set([...dbSlugs, ...fromFiles])];
    return combined;
  } catch {
    return fromFiles;
  }
...
// Carga cliente unificado: Supabase o .md
async function resolveClient(slug) {
  const dbClient = await loadClientFromDB(slug);
  if (dbClient) {
    return {
      slug,
      name:         dbClient.name         || 'Cliente',
      rubro:        dbClient.rubro         || '',
      colorPrimary: dbClient.color_primary || '#c8f135',
      colorAccent:  dbClient.color_accent  || '#888888',
      colorBtnText: dbClient.color_btn_text|| '#ffffff',
      mdContent:    dbClient.md_content    || null,
    };
  }
...
// ── AUTH HELPER — admin por Authorization header ──────────────
async function checkAdminAuth(req) {
  // Soporta: Authorization: Bearer <password> o query ?password= (legacy)
...
// ── CRUD CLIENTES EN SUPABASE ─────────────────────────────────
app.post('/api/admin/clients', async (req, res) => {
  if (!(await checkAdminAuth(req))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { slug, name, rubro, color_primary, color_accent, color_btn_text, md_content } = req.body;
    if (!slug || !/^[a-z0-9_-]+$/.test(slug)) return res.status(400).json({ error: 'Slug inválido' });
    await getDB().from('studio_clients').upsert({
      slug, name, rubro,
      color_primary:   color_primary  || '#c8f135',
      color_accent:    color_accent   || '#888888',
      color_btn_text:  color_btn_text || '#ffffff',
      md_content:      md_content     || null,
      updated_at: new Date().toISOString(),
    }, { onConflict: 'slug' });
    res.json({ ok: true });
...
app.delete('/api/admin/clients/:slug', async (req, res) => {
  if (!(await checkAdminAuth(req))) return res.status(401).json({ error: 'No autorizado' });
  try {
    await getDB().from('studio_clients').delete().eq('slug', req.params.slug);
    res.json({ ok: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/admin/billing/:slug', async (req, res) => {
  try {
    ...
    if (sale_price_usd   != null) update.sale_price_usd   = parseFloat(sale_price_usd) || 0;
