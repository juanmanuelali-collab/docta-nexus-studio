...
// Precios reales de los modelos (USD por token / por segundo GPU)
const MODEL_PRICES = {
  'claude-opus-4-5':       { input: 15.00 / 1_000_000, output: 75.00 / 1_000_000 },
  'claude-sonnet-4-20250514': { input: 3.00 / 1_000_000, output: 15.00 / 1_000_000 },
  'claude-haiku-4-5-20251001':{ input: 0.80 / 1_000_000, output: 4.00  / 1_000_000 },
  'flux-2-pro':            { per_second: 0.0032 }, // ~$0.05 en 15s
};
...
function calcFluxCost(seconds) {
  return (seconds || 15) * MODEL_PRICES['flux-2-pro'].per_second;
}
...
function initClientBilling(slug) {
  return {
    slug,
    plan_monthly: 30,
    pricing_mode: 'markup', // 'markup' | 'fixed' | 'free'
    markup: 4,
    fixed_price: { ars: 3000, usd: 2.5, eur: 2.3 },
    currency_display: 'ars',
    active: true,
    current_month: initMonth(),
    history: []
  };
}
...
function ensureCurrentMonth(billing) {
  ...
  if (!billing.current_month || billing.current_month.month !== cm) {
    // Archivar mes anterior en historial
    if (billing.current_month && billing.current_month.graphics > 0) {
      if (!billing.history) billing.history = [];
      billing.history.push(billing.current_month);
    }
    billing.current_month = initMonth();
  }
  return billing;
}
...
function checkBillingLimit(slug) {
  ...
  if (!b.active) return { ok: false, reason: 'inactive', billing: b };
...
function recordGeneration(slug, costs) {
  ...
  // Calcular costo real
  const realCost = {
    claude_analysis: costs.claude_analysis
      ? calcClaudeCost(costs.claude_analysis.model, costs.claude_analysis.input_tokens, costs.claude_analysis.output_tokens) : 0,
    flux_image: costs.flux ? calcFluxCost(costs.flux.seconds) : 0,
    claude_copy: costs.claude_copy
      ? calcClaudeCost(costs.claude_copy.model, costs.claude_copy.input_tokens, costs.claude_copy.output_tokens) : 0,
  };
  const totalRealUsd = Object.values(realCost).reduce((a, b) => a + b, 0);

  // Calcular precio cobrado al cliente
  let chargedUsd = 0;
  if (billing.pricing_mode === 'markup') {
    chargedUsd = totalRealUsd * (billing.markup || 1);
  } else if (billing.pricing_mode === 'fixed') {
    chargedUsd = billing.fixed_price?.usd || 0;
  }
  // 'free' → chargedUsd = 0
  ...
  cm.real_cost_usd = parseFloat(((cm.real_cost_usd || 0) + totalRealUsd).toFixed(6));
  cm.charged_usd   = parseFloat(((cm.charged_usd || 0) + chargedUsd).toFixed(6));

  // Agregar detalle (máx 200 entradas por mes)
  if (!cm.detail) cm.detail = [];
  cm.detail.push({
    date: new Date().toISOString(),
    real_cost_usd: parseFloat(totalRealUsd.toFixed(6)),
    real_cost_breakdown: realCost,
    charged_usd: parseFloat(chargedUsd.toFixed(6)),
    tokens: {
      claude_analysis: costs.claude_analysis ? { in: costs.claude_analysis.input_tokens, out: costs.claude_analysis.output_tokens } : null,
      claude_copy:     costs.claude_copy     ? { in: costs.claude_copy.input_tokens,     out: costs.claude_copy.output_tokens     } : null,
      flux_seconds:    costs.flux ? costs.flux.seconds : null,
    }
  });
  ...
  // Calcular montos en otras monedas
  ...
  return {
    ...
    real_cost_usd: parseFloat(totalRealUsd.toFixed(6)),
    real_cost_breakdown: realCost,
    charged_usd: parseFloat(chargedUsd.toFixed(4)),
    charged_ars: parseFloat((chargedUsd * rates.usd_to_ars).toFixed(0)),
    charged_eur: parseFloat((chargedUsd * rates.usd_to_eur).toFixed(2)),
  };
}

// Endpoint: estado de billing para el cliente (solo lo que ve el cliente)
app.get('/api/billing/:slug', (req, res) => {
  const billing = loadClientBilling(req.params.slug);
...
// Endpoint: registrar gráfica completa
app.post('/api/complete-graphic/:slug', (req, res) => {
  ...
  res.json({
    ...
    real_cost_usd: result.real_cost_usd,
    real_cost_breakdown: result.real_cost_breakdown,
    charged_usd: result.charged_usd,
    charged_ars: result.charged_ars,
    charged_eur: result.charged_eur,
  });
});

// Endpoint: admin — todos los clientes
app.get('/api/admin/billing', (req, res) => {
  ...
  const clients = files.map(f => {
    ...
    let billing = JSON.parse(fs.readFileSync(path.join(dir, f), 'utf8'));
    billing = ensureCurrentMonth(billing);
    ...
    const totalRealUsd   = allMonths.reduce((s, m) => s + (m.real_cost_usd || 0), 0);
    const totalChargedUsd = allMonths.reduce((s, m) => s + (m.charged_usd || 0), 0);
    const totalGraphics  = allMonths.reduce((s, m) => s + (m.graphics || 0), 0);
    ...
    return {
      ...
      current_month: {
        ...cm,
        charged_ars: parseFloat(((cm.charged_usd||0) * rates.usd_to_ars).toFixed(0)),
        charged_eur: parseFloat(((cm.charged_usd||0) * rates.usd_to_eur).toFixed(2)),
        real_cost_ars: parseFloat(((cm.real_cost_usd||0) * rates.usd_to_ars).toFixed(0)),
      },
      history: (billing.history || []).map(m => ({
        ...m,
        charged_ars: parseFloat(((m.charged_usd||0) * rates.usd_to_ars).toFixed(0)),
        charged_eur: parseFloat(((m.charged_usd||0) * rates.usd_to_eur).toFixed(2)),
        real_cost_ars: parseFloat(((m.real_cost_usd||0) * rates.usd_to_ars).toFixed(0)),
      })),
      totals: {
        graphics: totalGraphics,
        real_cost_usd: parseFloat(totalRealUsd.toFixed(4)),
        real_cost_ars: parseFloat((totalRealUsd * rates.usd_to_ars).toFixed(0)),
        charged_usd: parseFloat(totalChargedUsd.toFixed(4)),
        charged_ars: parseFloat((totalChargedUsd * rates.usd_to_ars).toFixed(0)),
        charged_eur: parseFloat((totalChargedUsd * rates.usd_to_eur).toFixed(2)),
        margin_usd: parseFloat((totalChargedUsd - totalRealUsd).toFixed(4)),
      }
    };
  });
...
app.post('/api/admin/billing/:slug', (req, res) => {
  ...
  billing = ensureCurrentMonth(billing);
  if (plan_monthly    !== undefined) billing.plan_monthly    = parseInt(plan_monthly);
  if (pricing_mode    !== undefined) billing.pricing_mode    = pricing_mode;
  if (markup          !== undefined) billing.markup          = parseFloat(markup);
  if (fixed_price     !== undefined) billing.fixed_price     = fixed_price;
  if (currency_display!== undefined) billing.currency_display = currency_display;
  if (active          !== undefined) billing.active          = active;
  if (reset_month) {
    // Archivar mes actual si tiene datos y empezar uno nuevo
...
app.post('/api/admin/config', (req, res) => {
  ...
  if (admin_password) cfg.admin_password = admin_password;
