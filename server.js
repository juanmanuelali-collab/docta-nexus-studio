...
Sos un director de arte experto en campanas de ecommerce y paid media para redes sociales.
...
## Imagen de referencia (si fue provista)

Si el usuario incluyo una imagen de referencia, analizala cuidadosamente:
- Identifica el estilo fotografico, paleta, composicion y mood
- Incorpora esos elementos en el prompt del fondo
- No copies la imagen: inspirate en su estetica para crear algo original pero coherente

## Zonas seguras (si fueron definidas)

Si el userPrompt incluye instrucciones de ZONAS SEGURAS, respotalas en el prompt:
- Indica a FLUX que deje esas zonas libres, con fondo neutro o gradiente suave
- Las zonas seguras son areas donde el usuario colocara textos o logos

## Como construir el prompt de fondo para FLUX Kontext

- En ingles, descriptivo y evocador, rico en detalles visuales
- Respeta las zonas seguras si fueron definidas
...
Responde UNICAMENTE con este JSON valido (sin markdown, sin backticks):
{
  ...
  "negative_prompt": "text, logos, watermarks, low quality, blur, distortion",
  "cta_style": {
  ...
  const refNote = hasRef ? '\nImagen de referencia incluida (3ra imagen): analizala para extraer estilo, paleta y mood.' : '';

  const msg = `Descripcion del usuario: ${userPrompt || 'No especificada'}${refNote}
CTAs EXACTOS (inmutables): ${ctas.join(' | ')}
Red social: ${networkLabels[network] || 'Instagram Feed'}
Logo incluido: ${logoBase64 ? 'Si' : 'No'}
Imagen de referencia: ${hasRef ? 'Si' : 'No'}`;
  ...
  const imageCount = 1 + (logoBase64 ? 1 : 0) + (hasRef ? 1 : 0);
  const imageLabels = ['1ra imagen: foto del producto (NO modificar)'];
  if (logoBase64) imageLabels.push('2da imagen: logo del cliente');
  if (hasRef) imageLabels.push((logoBase64 ? '3ra' : '2da') + ' imagen: REFERENCIA VISUAL (extraer estilo/mood/paleta)');
  content.push({ type: 'text', text: `(${imageLabels.join('. ')})\n\n${msg}` });
...
app.post('/api/generate-image', async (req, res) => {
  try {
    ...
    const r = await fetch('https://api.replicate.com/v1/models/google/gemini-2.5-flash-image/predictions', {
      ...
      body: JSON.stringify({ input: { prompt, aspect_ratio: ASPECT_RATIOS[network] || '1:1', output_format: 'jpg', output_quality: 95, safety_tolerance: 5, ...(negative_prompt && { negative_prompt }) } })
    });
    const data = await r.json();
