...
// ── VIDEO — imagen a video con Kling ──────────────────────────
// Recibe la pieza generada y la anima con movimiento cinematico
app.post('/api/generate-video', async (req, res) => {
  try {
    ...
    const input = {
      prompt: prompt || 'cinematic product advertisement, smooth camera movement, professional quality',
      image: 'data:image/jpeg;base64,' + imageBase64,
      duration: duration || 5,
      aspect_ratio: '1:1',
      negative_prompt: 'blur, distortion, text overlay, watermark, low quality',
      cfg_scale: 0.5,
    };

    const r = await fetch('https://api.replicate.com/v1/models/klingai/kling-v1-6-pro/predictions', {
      method: 'POST',
      headers: {
        'Authorization': 'Bearer ' + REPLICATE_TOKEN,
        'Content-Type': 'application/json',
